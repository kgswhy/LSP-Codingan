...
@section('title', 'Cek Pengaduan | SMK Telkom Jakarta')
@section('content')
                    ...
                    <div
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3">
                            @forelse ($pengaduan as $item)
                                <div class="max-w-2xl mt-5 mx-5 px-3 py-4 bg-white rounded-lg shadow-lg ">
                                    <div class="flex justify-between items-center">
                                        <span
                                            class="font-light text-gray-600 text-gray-400 text-sm">{{ $item->created_at->format('d/m/Y') }}</span>
                                        <p
                                            class="px-3 m-1 m py-1 bg-gray-600 text-gray-100 text-sm font-bold rounded hover:bg-gray-500">
                                                ...
                                                @if ($item->status === 'pending')
                                                    <div class="flex">
                                                        <a href="{{ route('pengaduan.edit', Crypt::Encrypt($item->id)) }}"
                                                            class="inline-flex items-center w-full px-6 py-3 text-sm font-medium leading-4 text-white transition duration-150 ease-in-out bg-transparent bg-red-600 border border-transparent md:px-3 md:w-auto md:rounded-md mt-5 lg:px-5 hover:bg-indigo-500 focus:outline-none focus:border-indigo-700 focus:shadow-outline-indigo active:bg-red-700">Ubah</a>
                                                        <form action="{{ Route('pengaduan.delete', $item->id) }}"
                                                            method="post">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
                                                                class="inline-flex items-center w-full px-6 py-3 text-sm font-medium leading-4 text-white transition duration-150 ease-in-out bg-transparent bg-red-600 border border-transparent md:px-3 md:w-auto md:rounded-md mt-5 lg:px-5 hover:bg-red-500 focus:outline-none focus:border-red-700 focus:shadow-outline-red active:bg-red-700 ml-2">Hapus</button>
                                                        </form>
                                                    </div>
                                                @endif
                                            @endif
